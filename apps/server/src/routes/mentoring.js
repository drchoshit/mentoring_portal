...
export default function mentoringRoutes(db) {
  ...
  router.get('/record', (req, res) => {
    ...
    res.json({
      student: sanitizeStudentForRole(student, req.user.role),
      week,
      subjects,
      subject_records,
      week_record,
    ...
    for (const row of rows) {
      ...
      for (let problemIndex = 0; problemIndex < mergedProblems.length; problemIndex += 1) {
        ...
        assignments.push({
          ...
          assigned_by: String(assignment.assigned_by || '').trim(),
          submitted_at: String(problem.submitted_at || '').trim(),
          submitted_by: String(problem.submitted_by || '').trim(),
